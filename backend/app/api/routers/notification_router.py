...
async def mark_notification_as_read(
    ...
    service = get_notification_service(request)
    normalized_notification_id = normalize_notification_id(notification_id)

    try:
        return service.mark_as_read(current_user, normalized_notification_id)
...
async def notifications_ws(websocket: WebSocket):
    service = get_notification_service_from_websocket(websocket)
    if service is None:
