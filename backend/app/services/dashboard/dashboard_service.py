...
class DashboardService:
    ...
    def build_admin_dashboard(self) -> dict:
        ...
        all_users = self.users_repository.list_all()
        recent_sessions = self.sessions_repository.list_recent(limit=80)

        total_documents = self.documents_repository.count_all()
        indexed_count = self.documents_repository.count_all(status="indexed")
        processing_count = self.documents_repository.count_all(status="processing")
        failed_count = self.documents_repository.count_all(status="failed")
