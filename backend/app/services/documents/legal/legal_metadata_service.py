...
class LegalMetadataService:
    ...
    def build_legal_metadata(
        ...
        *,
        ...
        if normalized_relation_type == LegalRelationType.none.value:
            ...
            raise HTTPException(
                status_code=400,
                detail="targetDocumentId est requis lorsqu une relation juridique est renseignee.",
            )

        if normalized_related_document_id is not None:
            self.validate_target_exists(normalized_related_document_id)
    ...
    def validate_target_exists(self, document_id: str) -> None:
        ...
        if not normalized_document_id:
            raise HTTPException(status_code=400, detail="targetDocumentId est invalide.")
    ...
    def _validate_relation_type(relation_type: str) -> None:
        ...
        if relation_type not in allowed:
            raise HTTPException(
                status_code=400,
                detail="relationToTarget doit etre une valeur relationnelle valide.",
            )
