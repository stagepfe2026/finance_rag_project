...
export default function DashboardLatestAccessCard({ items }: { items: AdminDashboardLatestAccess[] }) {
  ...
  return (
    <div className="rounded-lg border border-[#e0e6f0] bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-[11px] uppercase tracking-[0.12em] text-[#6c7894]">Acces recents</p>
          <h2 className="mt-1 text-[18px] font-bold text-[#071f3d]">5 derniers acces</h2>
        </div>
      ...
      <div className="mt-4 space-y-2">
        {latestItems.length === 0 ? (
          ...
          latestItems.map((access) => (
            <div key={access.userId} className="rounded-lg border border-[#e8edf5] bg-white px-3 py-2.5 transition hover:border-[#f3c6cc] hover:bg-[#fffafd]">
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-center gap-2.5">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#9d0208] text-[11px] font-bold text-white">
                    {access.userName.charAt(0).toUpperCase()}
                  </span>
                  <span className="min-w-0">
                    <p className="truncate text-[13px] font-bold text-[#071f3d]">{access.userName}</p>
                    <p className="mt-0.5 truncate text-[11px] text-[#5f6680]">{access.email}</p>
                  </span>
                </div>
              ...
              <div className="mt-1.5 flex items-center justify-between text-[11px] text-[#5f6680]">
