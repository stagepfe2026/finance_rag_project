...
export default function TrendChart({ trend, isLoading }: TrendChartProps) {
  ...
  const series = useMemo(
    () => [
      { name: "Likes", data: trend.map((point) => point.likes) },
      { name: "Dislikes", data: trend.map((point) => point.dislikes) },
      { name: "Signalements", data: trend.map((point) => point.signalements) },
    ],
    [trend],
  );
  ...
  return (
    <section className="rounded border border-[#e5eaf2] rounded-lg bg-white">
      ...
      <div className="px-2 pb-2 pt-3">
        {isLoading ? (
          <div className="flex h-[260px] items-center justify-center text-[12px] text-[#8a96ad]">Chargement...</div>
        ) : trend.length === 0 ? (
